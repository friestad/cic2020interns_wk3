...
public class Covid19ApiServiceImpl implements Covid19ApiService {
	...
	public Comparison getComparison(List<Country> countries, CompareBy compareBy) {
		...
		try {
			String newstr = compareBy.toString();
			
			if(newstr.equals("TotalRecovered")) {
			countries.sort((Country c1, Country c2)->c1.getTotalRecovered()-c2.getTotalRecovered());	
			}
			else if(newstr.equals("TotalDeaths")) {
				countries.sort((Country c1, Country c2)->c1.getTotalDeaths()-c2.getTotalDeaths());
			}
			else if(newstr.equals("TotalConfirmed")) {
				countries.sort((Country c1, Country c2)->c1.getTotalConfirmed()-c2.getTotalConfirmed());
			}
